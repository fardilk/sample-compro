  ...
  return (
    <>
      ...
      >
        <nav className="mx-auto" style={{ width: '90%' }}>
          <div className="grid grid-cols-[20%_auto_auto] items-center h-[6em] w-full">
            ...
            <div className="flex items-center min-w-[10rem]">
              <img src="/img/logo-excellence-plus-indonesia.png" alt="Excellence Plus Indonesia" className="h-auto w-[10rem]" />
            </div>
              ...
              <div className="hidden md:flex w-full gap-4">
                <div className="flex gap-2 whitespace-nowrap w-full">
                  {mainMenu.map((item, idx) => (
                    <button
                      key={item.label}
                      className={`flex items-center px-4 py-2 text-gray-800 hover:text-blue-600 rounded-md text-sm font-medium focus:outline-none whitespace-nowrap ${openMenuIdx === idx ? 'bg-gray-100' : ''}`}
        ...
        <div className="w-full py-2 text-center bg-gradient-to-r from-blue-600 to-purple-600">
          <div className="w-full flex justify-center items-center text-center bg-transparent">
            ...
            <span className="text-white underline cursor-pointer px-3 py-1 rounded">
              Book consultation!
            </span>
          </div>
        </div>
      ...
      {openMenuIdx !== null && (
        <div className="fixed left-0 top-16 w-full h-[calc(100vh-4rem)] bg-white z-50 shadow-lg flex">
          ...
          {/* Canvas right side (empty for now, can add content later) */}
          <div className="flex-1 h-full">
            ...
            {openMenuIdx !== null && activeSubIdx !== null && mainMenu[openMenuIdx].children?.[activeSubIdx]?.children && (
              <HoverMenu2 items={mainMenu[openMenuIdx].children[activeSubIdx].children!} />
            )}
            {/* Show cards for leaf menu items (no children, but have description/icon) */}
            {openMenuIdx !== null && activeSubIdx !== null &&
              ...
              !mainMenu[openMenuIdx].children?.[activeSubIdx]?.children && (
                <HoverMenu2 items={[mainMenu[openMenuIdx].children![activeSubIdx]]} />
            )}
          </div>
        </div>
      )}
